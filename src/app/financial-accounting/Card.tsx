...
const CardAccounting = ({icon, title, subtitle, index}: Props) => {
  ...
  return (
    <motion.div className={`flex flex-col justify-between rounded-[20px] overflow-hidden ${index == 0 && 'sm:bg-[#F8F5FF] border sm:border-[#E2D4FF]'} ${index == 1 && 'sm:bg-[#FFF8E5] border sm:border-[#FFF8BB]'}`}>
      <motion.div variants={variants} className="p-4 sm:p-8">
        ...
        <img src={icon} alt="payable" className={`${index ==  0 && 'sm:pl-6'} ${index == 1 && 'sm:pr-6' }`} />
    </motion.div>
  )
}
