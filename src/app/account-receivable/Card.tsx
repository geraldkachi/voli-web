...
const CardReceivable = ({icon, title, subtitle, keyidx}: Props) => {
  ...
  return (
    <motion.div  className="flex flex-col justify-between rounded-[20px] bg-[#F8F8F8]">
      <motion.div variants={variants} className='p-4 md:p-8'>
        ...
        <img src={icon} alt="payable" className={`mt-4 ${keyidx === 0  && '!-mt-20'} ${keyidx == 1 && 'self-start'}  ${keyidx === 3 && '!-mt-20'}`} />
    </motion.div>
  )
}
