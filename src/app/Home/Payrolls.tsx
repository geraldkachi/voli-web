  ...
  return (
    <motion.div  className="flex flex-col justify-between  rounded-[20px] bg-white overflow-hidden" onMouseEnter={() => setSelectedOption(index)} onMouseLeave={() => setSelectedOption(9)}>
      <motion.div variants={variants} className="p-4 md:p-8">
        ...
        <Button title="Learn More" className={`${selectedOption == index && '!bg-black !text-white'} !sm:flex !bg-[#F2F2F3] !text-[#00000A] mt-5`}
        suffixIcon={<>
         {selectedOption != index ? <svg className="ml-5" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M1 15L15 1M15 1H5.66667M15 1V10.3333" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
       </svg> 
        }
            </>}
        />
